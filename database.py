...
class DatabaseManager:
    ...
    def setup_database(self):
        ...
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_outlook_id ON eventos_sincronizados(outlook_event_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_google_id ON eventos_sincronizados(google_event_id)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_expresso_id ON eventos_sincronizados(expresso_event_id)')
    ...
    def store_google_event(self, event):
        ...
        if exists:
            ...
            """, (
                ...
                'dateTime' not in event.get('start', {}),  # se não tem dateTime, é all day
    ...
    def map_events(self, outlook_id=None, google_id=None, expresso_id=None, origem=None):
        ...
        if outlook_id:
            cursor.execute("SELECT id FROM eventos_sincronizados WHERE outlook_event_id = ?", (outlook_id,))
            mapping = cursor.fetchone()
            if mapping:
                mapping_id = mapping[0]
        
        if google_id and not mapping_id:
            cursor.execute("SELECT id FROM eventos_sincronizados WHERE google_event_id = ?", (google_id,))
            mapping = cursor.fetchone()
            if mapping:
                mapping_id = mapping[0]
        
        if expresso_id and not mapping_id:
            cursor.execute("SELECT id FROM eventos_sincronizados WHERE expresso_event_id = ?", (expresso_id,))
            mapping = cursor.fetchone()
            if mapping:
                mapping_id = mapping[0]
    ...
    def cleanup_old_events(self, days_to_keep=0):
        ...
        cursor = self.conn.cursor()
        now = datetime.now()
        cutoff_date = (now - timedelta(days=days_to_keep)).replace(hour=0, minute=0, second=0, microsecond=0).isoformat()
        ...
        cursor.execute("DELETE FROM google_events WHERE end_datetime < ? AND end_datetime != ''", (cutoff_date,))
        deleted_google = cursor.rowcount
        
        cursor.execute("DELETE FROM outlook_events WHERE end_datetime < ? AND end_datetime != ''", (cutoff_date,))
        deleted_outlook = cursor.rowcount
        
        cursor.execute("DELETE FROM expresso_events WHERE data_fim < ? AND data_fim != ''", (cutoff_date,))
